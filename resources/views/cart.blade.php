...
@section('title', 'Shopping Cart - Travelia')
			...
			<div class="col-lg-8" data-aos="fade-up">
				<div class="tt-cart-table">
					...
					<div class="tt-cart-item">
						<div class="tt-cart-col-product">
							<div class="d-flex align-items-center gap-3">
								<img src="{{ $destinations->image ? asset('storage/' . $destinations->image) : asset('images/destination-1.jpg') }}"
									 alt="{{ $destinations->title }}" class="tt-cart-img" loading="lazy">
								<div>
									<h6 class="mb-1">{{ $destinations->title }}</h6>
									<small class="text-muted">{{ $destinations->category->name ?? 'Tour' }}</small>
								</div>
							</div>
						</div>
						<div class="tt-cart-col-price" data-label="Price">{{ $destinations->converted_pricing }}</div>
						...
						<div class="tt-cart-col-total" data-label="Total">{{ $destinations->converted_pricing }}</div>
					</div>
				</div>
				...
				<div class="d-flex flex-wrap justify-content-between align-items-center mt-4 gap-3">
					...
					<div class="d-flex gap-2">
						<form action="{{ route('cart.remove', $destinations->id ?? '') }}" method="POST">
							@csrf
							@method('DELETE')
			...
			<div class="col-lg-4" data-aos="fade-left">
				<div class="tt-sidebar-card">
					...
					<div class="d-flex justify-content-between mb-2">
						<span>Subtotal</span>
						<strong>{{ $destinations->converted_pricing }}</strong>
					</div>
					<div class="d-flex justify-content-between mb-2">
