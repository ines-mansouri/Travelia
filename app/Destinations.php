...
class Destinations extends Model
{
    ...
    protected $fillable = [
        'title', 'description', 'content', 'image', 'published_at', 'category_id', 'pricing', 'currency',
        'duration', 'group_size', 'tour_type', 'latitude', 'longitude',
    ];
    ...
    public function pilgrimageDetail(): HasOne
    {
        return $this->hasOne(PilgrimageDetail::class, 'destination_id');
    }

    public function expenses(): HasMany
    {
        return $this->hasMany(Expense::class, 'destination_id');
    }
    ...
    {
        return $this->revenue - $this->expenses_total;
    }

    public function getFormattedPricingAttribute(): string
    {
        return $this->formatPrice($this->currency, $this->price);
    }

    public function getConvertedPricingAttribute(): string
    {
        $selected = session('currency', config('currencies.default'));
        if ($selected === $this->currency) {
            return $this->formatted_pricing;
        }

        $cacheKey = "dest.pricing.{$this->id}.{$selected}";

        return Cache::remember($cacheKey, now()->addMinutes(10), function () use ($selected) {
            $converted = $this->convertTo($selected);
            $symbol = config("currencies.symbols.$selected", "$selected ");
            return $symbol . number_format($converted, 0);
        });
    }

    public function convertTo(string $targetCurrency): float
    {
        $rates = config('currencies.rates', []);
        $baseRate = $rates[$this->currency] ?? 1;
        $targetRate = $rates[$targetCurrency] ?? 1;
        $priceInEur = $this->price / $baseRate;
    ...
    private function formatPrice(string $currency, float $price): string
    {
